...
### 一次性任务结果
"X 已生成完成"、"Y 已成功"、"翻译完成"、"路线已规划"、"PPT 已导出"、"整理了 N 个文件" —— 任务做完就过去，下次不参考。

### 临时状态
"API 密钥无效"、"X 测试成功"、"端口被占用"、"服务连不上"、"持久化验证通过" —— 都是即时状态，下次未必成立。

### 可派生信息
"项目位于 /Users/X"、"应用名叫 Y"、"项目用 Tauri/React"、"代码在 N 处用了 Z 模式" —— 从工作区路径、package.json、grep 都能推断，不需要记。
